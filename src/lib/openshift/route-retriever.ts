...
export class OpenShiftRouteRetriever {
   ...
   */
  public async getRoutesFromNamespaces(
    namespaces: readonly string[]
  ): Promise<Result<RouteInfo[], Error>> {
    ...
    for (const namespace of namespaces) {
      ...
      if (result.ok) {
        ...
        errors.push(
          `Failed to get routes from namespace ${namespace}: ${result.error.message}`
        );
      }
    }
  ...
  private async getRoutesFromNamespace(
    ...
    try {
      ...
      const parsedRoutes = routes.map((route: any) => parseRoute(route));
